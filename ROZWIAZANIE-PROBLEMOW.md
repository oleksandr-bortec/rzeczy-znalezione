...
### Виправлення проблеми з автооновленням кешу

#### Проблема
Незважаючи на наявність даних в базі, кеш не оновлювався автоматично через:
1. Відсутність обробника повідомлення `SKIP_WAITING` у Service Worker
2. Неоптимальну періодичну перевірку оновлень

#### Виправлення
1. ✅ Додано обробник `message` event у [sw.js:122-129](sw.js#L122-L129)
2. ✅ Покращено механізм періодичної перевірки з детальним логуванням
3. ✅ Змінено інтервал перевірки: 30 секунд для localhost, 5 хвилин для production
4. ✅ Оновлено версію до 2.0.7 для синхронізації SW та сервера
...
# 3. Відкрити Developer Tools → Console
# 4. Перевірити логи:
#    [SW] Update check scheduled every 30s
#    [SW] Checking for updates...
#    [SW] Server version: 2.0.7, Current version: 2.0.7

# 5. Або викликати вручну:
await window.checkForUpdates()
```
